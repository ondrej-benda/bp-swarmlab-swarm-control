    ...
    for agent = 1:nb_agents
        ...
        if nb_neig ~= 0
            ...
            for agent2 = neig_list'
                ...
                elseif self.as_swarm==true  % attraction
                    vel_rep(:, agent) = vel_rep(:, agent) + ...
        ...
        if self.as_swarm == true
%             vel_command(:, agent) = vel_rep(:, agent) + vel_obs(:, agent) + vel_wall(:, agent)+ vel_fric(:, agent);
            vel_command(:, agent) = vel_rep(:, agent) + vel_obs(:, agent)+ vel_fric(:, agent);
        else
            vel_command(:, agent) = vel_rep(:, agent) + vel_obs(:, agent)+ vel_fric(:, agent);
        end
        ...
        elseif p_swarm.is_active_goal == true
            goal_rel = self.drones(agent).waypoints(1:3) - pos(:, agent);
            u_goal = goal_rel / norm(goal_rel);
