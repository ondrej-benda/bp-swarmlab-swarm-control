...
pocet_sad=24;
pocet_map=10;
rozsah_parametru=[p_sim.dt,10;0,3;0,50;5,100];
...
for j=1:pocet_map
    ...
    mapy(j).gass_center=[map.width*rand();map.width*round(rand())];
end
...
while true
    ...
    pso.frequency=hodnoty_parametru(iterace,1);
    pso.frequency=pso.frequency-mod(pso.frequency,p_sim.dt);
    
    pso.we=hodnoty_parametru(iterace,2);
    pso.r=hodnoty_parametru(iterace,3);
    pso.radius_exploring=hodnoty_parametru(iterace,4);
