...
while true
    ...
    for mapa=1:pocet_map
        ...
        map=mapy(mapa);
        p_swarm.P0 = [is_in_object(map,[0;map.width/2]);-50];
...
%         map.gass_center=is_in_object(map,map.gass_center);
        ...
        prubeh_dron(:,mapa,iterace)=[cas_konce,sum(swarm.collisions_history(50:end,1)),sum(swarm.collisions_history(50:end,2))];
        disp(iterace)
        disp(prubeh_dron(:,mapa,iterace))
    end
    if iterace==1
