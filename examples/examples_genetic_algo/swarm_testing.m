...
body=zeros(pocet_sad,1);
soudrznost=zeros(pocet_sad,1);
...
hist_body=zeros(pocet_generaci,pocet_sad);
hist_soudrznost=zeros(pocet_generaci,pocet_sad);
...
%         map.gass_center=is_in_object(map,map.gass_center);
        ...
        for time = p_sim.start_time:p_sim.dt:p_sim.end_time
            ...
            if swarm.as_swarm
                if swarm.as_swarm_reached_goal
                    goal=rand(2,1)*map.width;
                    goal=is_in_object(map,goal);
                    swarm.as_swarm_to_goal(goal);
                end
            end
        ...
        if swarm.pocet_splnenych~=0
            swarm.pocet_splnenych=swarm.pocet_splnenych-1;
        end
        
        %Evolucni algo
        swarm.collisions_history(1:50,1:2)=zeros(50,2);
        soudrznost=spocti_soudrznost(swarm.get_pos_ned_history());
        pocet_bodu=swarm.pocet_splnenych;
        prubeh_dron(:,mapa,iterace)=[pocet_bodu;spocti_soudrznost(swarm.get_pos_ned_history());sum(swarm.collisions_history(:,1));sum(swarm.collisions_history(:,2))];
        fitness_mapy=0;
            soudrznost_sady=soudrznost_sady+spocti_soudrznost(swarm.get_pos_ned_history());
            disp([num2str(generace),'.',num2str(iterace),'.',num2str(mapa),':Kolize_agent: ',num2str(sum(swarm.collisions_history(:,1))),'Kolize_obs: ',num2str(sum(swarm.collisions_history(:,2))),' Body (',num2str(swarm.pocet_splnenych),')']);
...
%     soudrznost(iterace)=soudrznost_sady;
%     soudrznost_sady=0;
%     body(iterace)=fitness_sady;
%     disp([num2str(generace),'.',num2str(iterace),' Body_sady: ',num2str(body(iterace)),' Soudrznost_sady: ',num2str(soudrznost(iterace))]);
%     if iterace==pocet_sad
%             fitness=spocti_fitness_roj(body,soudrznost);
%             disp('');
...
%             hist_soudrznost(generace,:)=soudrznost';
%             hist_body(generace,:)=body';
%         
%             generace=generace+1;
%             iterace=0;
