...
%         map.gass_center=is_in_object(map,map.gass_center);
    ...
    %Vyhodnoceni
        swarm.collisions_history(1:10,1:2)=zeros(10,2);
        pocet_bodu=0;
        for dron=1:swarm.nb_agents
            pocet_bodu=pocet_bodu+swarm.drones(dron).pocet_splnenych;
        end
        fitness_mapy(mapa,iterace)=spocti_fitness_dron(swarm);
        prubeh_dron(:,mapa,iterace)=[spocti_fitness_dron(swarm);pocet_bodu;sum(swarm.collisions_history(:,1));sum(swarm.collisions_history(:,2))];
        disp([num2str(generace),'.',num2str(iterace),'.',num2str(mapa),':Kolize_agent: ',num2str(sum(swarm.collisions_history(:,1))),'Kolize_obs: ',num2str(sum(swarm.collisions_history(:,2))),' Body ',num2str(spocti_fitness_dron(swarm))]);
        if ~evolucni_ladeni
            break;
        end
    end
